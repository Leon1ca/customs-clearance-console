...
internal sealed class DetailForm : Form
{
    ...
    public DetailForm(DeclarationRecord record)
    {
        ...
        var amountHeader = record.Totals.Keys.OrderBy(x => x, StringComparer.Ordinal).FirstOrDefault()
            ?? record.LineTotals.Select(x => x.Currency).FirstOrDefault(x => !string.IsNullOrWhiteSpace(x)) ?? "原币";
        var columnHeader = new DetailColumnHeader($"总价（{amountHeader}）");
        ...
        var footerPanel = new Panel { Dock = DockStyle.Bottom, Height = 84, BackColor = UiTokens.Colors.DialogFooter };
    ...
    private sealed class DetailLineList : Control
    {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            var index = e.Y / size;
            var next = index >= 0 && index < _record.LineTotals.Count ? index : -1;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            var rowHeight = S(44);
            var lines = _record.LineTotals.OrderBy(x => x.PageNumber).ThenBy(x => x.Sequence).ToList();
            if (lines.Count == 0)
            {
    ...
    private sealed class DetailFooter : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            var primaryCurrency = _record.Totals.Keys.OrderBy(x => x, StringComparer.Ordinal).FirstOrDefault()
                ?? _record.LineTotals.Select(x => x.Currency).FirstOrDefault(x => !string.IsNullOrWhiteSpace(x)) ?? "";
            var amount = _record.Totals.Count > 0
                ? _record.Totals.OrderBy(x => x.Key, StringComparer.Ordinal).First().Value
                : _record.LineTotals.Where(x => x.IsReliable).Sum(x => x.Amount);
            using (var labelFont = Theme.UiFont(12.5F))
                TextRenderer.DrawText(graphics, $"共 {_record.LineTotals.Count} 项 · 已识别可靠分项合计", labelFont, new Rectangle(S(20), S(14), S(300), S(22)), Theme.Muted,
                    TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPadding);
            using (var currencyFont = Theme.UiFont(12F))
                TextRenderer.DrawText(graphics, primaryCurrency, currencyFont, new Rectangle(Width - S(200), S(14), S(50), S(24)), Theme.Muted,
                    TextFormatFlags.Right | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPadding);
            using (var amountFont = Theme.MonoFont(18F, true))
                TextRenderer.DrawText(graphics, amount.ToString("N2"), amountFont, new Rectangle(Width - S(150), S(10), S(130), S(30)),
                    consistent ? Theme.Text : Theme.Warning, TextFormatFlags.Right | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPadding);
            var icon = UiV2Icons.Load(consistent ? Ui2.CheckGreen : Ui2.Alert, 16, dpi);
            if (icon is not null)
            {
                graphics.DrawImage(icon, S(20), S(48), icon.Width, icon.Height);
                icon.Dispose();
            }
            using var messageFont = Theme.UiFont(12F);
            TextRenderer.DrawText(graphics, message, messageFont, new Rectangle(S(42), S(44), Width - S(62), S(30)),
                consistent ? Theme.Success : Theme.Warning,
                TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPadding | TextFormatFlags.WordBreak | TextFormatFlags.EndEllipsis);
        }
    }
    ...
    private static IEnumerable<(Rectangle Rect, string Text, ContentAlignment Alignment)> Columns(int width, int dpi, int y, int height, string amountHeader)
    {
        ...
        var padX = S(20);
        var gap = S(12);
        var available = width - padX * 2 - gap * 4;
        var number = S(36);
        var quantity = S(120);
        var unitPrice = S(90);
        var amount = S(130);
        var name = Math.Max(S(80), available - number - quantity - unitPrice - amount);
        var widths = new[] { number, name, quantity, unitPrice, amount };
        var x = padX;
        var result = new List<(Rectangle, string, ContentAlignment)>();
        var headers = new[] { "项号", "商品名称", "数量 / 单位", "单价", amountHeader };
        var alignments = new[] { ContentAlignment.MiddleLeft, ContentAlignment.MiddleLeft, ContentAlignment.MiddleRight, ContentAlignment.MiddleRight, ContentAlignment.MiddleRight };
        for (var i = 0; i < 5; i++)
        {
