// Layout-independent fingerprint of the queried result. It intentionally ignores
// document height (prepare grows scroll containers) so it can be compared before and
// after the capture: any real result or query change is detected, while the expand /
// restore of scroll containers is not mistaken for a change.
(() => {
  ...
  if (numbers.length === 0) return 'stale|no-result-number';
  ...
  return 'stable|' + (state.queryAt || 0) + ':' + resultText.length + ':' + hash(resultText);
})()
