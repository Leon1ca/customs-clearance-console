...
v1.4.0（2026-09-09 整合版）：将此前本地迭代统一归入 1.4 正式版本，保留当前全部功能，而非回退到旧代码。新增逐单 Markdown 列表导出，移除详情弹窗；核验流程支持自动填写单号、人工验证码查询后自动保存长截图。包含统一扁平 PNG 图标、精简复制菜单、标题大于金额的字号层级及 PDF 下划线位置修复。详见 [更新说明](docs/release-notes-v1.4.0.md)。历史迭代文档和设计目录保留用于追溯，不代表当前发布版本。
