...
v1.6.0：新增逐项金额详情，可按 PDF 页码和项号核对每一条识别总价及双引擎复核结果。修复多页关单末行漏算、“印度尼西亚”被截断、横向扫描页未自动旋转，以及混合 PDF 中放行通知书/装货单/委托协议被误当作关单累计的问题。金额双引擎冲突时不再擅自选取某一金额计入合计；当单一引擎完整读取价格表时，须通过连续页面和表格结构校验后才采用。
...
- 每条记录提供“详情”按钮，展示页码、项号、识别总价、复核金额和校对结论；不可靠金额保留供人工比对，但不计入合计。
...
- “开始识别”不访问网站；网站校验始终由用户逐条点击并完成人工验证，之后保存以报关单号命名的长截图。
